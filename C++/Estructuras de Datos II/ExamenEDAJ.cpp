...
template <typename T>
class UniqueChildIterator {
public:
    UniqueChildIterator() : t(), p() {}
    UniqueChildIterator(T tree) : t(tree), p() {p.push(t.getRoot());}
    
    UniqueChildIterator& operator++() {
            auto e = p.top();
            p.pop();
            auto d = t.getRightChild(e);
            auto i = t.getLeftChild(e);
            ...
            if (i)
                p.push(i);
            if ((i && !d) || (d && !i)) return *this;
        }
        
    UniqueChildIterator operator++(int) {
        auto tmp(*this);
        operator++();
        return tmp;
    }
    ...
    bool operator!=(const UniqueChildIterator& other) {
        //std::cout << "Es empty p" << p.empty() << "\n";
        //std::cout << "Es empty other" << other.p.empty() << "\n";
        bool d = (p.empty() != other.p.empty());
        //std::cout << "Distinto: " << d << "\n";
...
int main() {
    ...
    Iterable<LinkedBinaryTree<int>, UniqueChildIterator<LinkedBinaryTree<int>>> uniqit(t);
    for (int i : uniqit) {
        std::cout << i << " ";
    }
    std::cout << '\n';
